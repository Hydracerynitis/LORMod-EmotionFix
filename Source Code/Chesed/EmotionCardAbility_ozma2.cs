    ...
    public class EmotionCardAbility_ozma2 : EmotionCardAbilityBase
    {
        ...
        public override void OnSelectEmotion()
        {
            ...
            if (_owner.faction == Faction.Enemy)
                _owner.bufListDetail.AddBuf(new DiceCardSelfAbility_ozma_forgotten.BattleUnitBuf_ozma_forgotten());
            else
                _owner.bufListDetail.AddBuf(new Oblivion());
        }
        public override void OnRoundStart()
        {
            ...
            if (_owner.faction != Faction.Enemy && !_owner.bufListDetail.GetActivatedBufList().Exists(x => x is Oblivion))
        ...
        public void Destory()
        {
            BattleUnitBuf buff = _owner.bufListDetail.GetActivatedBufList().Find((Predicate<BattleUnitBuf>)(x => x is Oblivion));
            if (buff != null)
                buff.Destroy();
            buff = _owner.bufListDetail.GetActivatedBufList().Find(x => x is DiceCardSelfAbility_ozma_forgotten.BattleUnitBuf_ozma_forgotten);
            if (buff != null)
                buff.Destroy();
        }
        public class Oblivion: BattleUnitBuf
        {
            ...
            public override void OnRoundEnd()
            {
                ...
                if (count >= 3)
                    Destroy();
                if (BattleObjectManager.instance.GetAliveList(_owner.faction).FindAll(x => x != _owner).Count <= 0)
